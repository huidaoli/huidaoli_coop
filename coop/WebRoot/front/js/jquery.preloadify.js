...
$(function () {
	$.fn.preloadify = function (options) {
		var defaults = {delay:0, imagedelay:10, mode:"parallel", preload_parent:"div", check_timer:200, ondone:function () {
		}, oneachload:function (image) {
		}, fadein:700, force_icon:false};
	
	// variables declaration and precaching images and parent container
	    
		var options = $.extend(defaults, options), parent = $(this), timer, i = 0, j = options.imagedelay, counter = 0, 
		images = parent.find("img").css({display:"block", visibility:"hidden", opacity:0}), 
		checkFlag = [], 
		imagedelayer = function (image, time) {
			$(image).css("visibility", "visible").delay(time).animate({opacity:1}, options.fadein, function () {
				$(this).parent().removeClass("preloader");
			});
		};
		//alert(parent.find("img").length);
	...
	// keep on checking after predefined time, if image is loaded
		...
		if (options.force_icon == true) {
			...
			var icon = jQuery("<img />", {id:"loadingicon", src:src}).hide().appendTo("body");
			timer = setInterval(function () {
