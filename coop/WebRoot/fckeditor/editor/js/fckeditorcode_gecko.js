		...
		FCKTools.AddEventListener(FCK.EditorDocument, 'paste', function(evt) {
					...
					setTimeout(function() {
								...
								C = FCK.EditorDocument
										.getElementById('fck_paste_padding');
...
FCKConfig.ProcessHiddenField = function() {
	...
	for (var i = 0; i < B.length; i++) {
		...
		if (D == 'CustomConfigurationsPath')
			...
			this.PageConfig[D] = parseFloat(E);
		else
			this.PageConfig[D] = E;
	}
};
...
FCKXHtml.GetXHTML = function(A, B, C) {
	...
	if (FCKBrowserInfo.IsIE)
		FCKXHtml._RemoveXHtmlJobProperties(A);
...
FCKDialogCommand.prototype.Execute = function() {
	FCKDialog.OpenDialog('FCKDialog_' + this.Name, this.Title, this.Url,
			this.Width, this.Height, this.CustomValue, this.Resizable);
};
...
FCKSourceCommand.prototype.Execute = function() {
	if (FCKConfig.SourcePopup) {
		...
		FCKDialog.OpenDialog('FCKDialog_Source', FCKLang.Source,
				'dialog/fck_source.html', A, B, null, true);
	} else
		FCK.SwitchEditMode();
};
...
(function() {
	var A = [];
	var B = (FCK && FCK.EditorWindow && FCK.EditorWindow.parent.parent.scayt)
			? true
			: false;
	var C = false;
	var D = false;
	function ScaytEngineLoad(callback) {
		...
		if (FCK.Config.ScaytCustomUrl)
			FCK.Config.ScaytCustomUrl = new String(FCK.Config.ScaytCustomUrl)
					.replace(new RegExp("^http[s]*:\/\/"), "");
		var H = document.location.protocol;
		var I = FCK.Config.ScaytCustomUrl
	...
	T.prototype.Execute = function(c) {
		switch (c) {
			...
			case 'About' :
				...
				;
				if (B && D)
					FCKDialog.OpenDialog('Scayt', 'SCAYT Settings',
							'dialog/fck_scayt.html?' + c.toLowerCase(), 343,
							343);
				break;
			default :
				if (!B) {
	...
	function ShowScaytState() {
		...
		if (!W || !W._Combo || !W._Combo._OuterTable)
			return;
	...
	Z.prototype.GetLabel = function() {
		...
		return FCKBrowserInfo.IsIE
				? '<div class="TB_Button_Image"><img src="' + a
						+ '" style="top:-192px"></div>'
	...
	b.prototype.Execute = function(contextInfo) {
		var c = contextInfo && contextInfo.action, g = c && contextInfo.node, Q = window.scayt_control;
		if (g) {
			switch (c) {
				case 'Suggestion' :
	...
	function InitSetup() {
		FCK.ContextMenu.RegisterListener({
					AddItems : function(menu) {
						var E = FCK.EditorWindow.parent.parent;
						var Q = window.scayt_control, P = E.scayt;
						if (!Q)
							return;
	...
	FCK.Events.AttachEvent('OnAfterSetHTML', function() {
				if (FCKConfig.SpellChecker == 'SCAYT') {
					if (!B && FCK.Config.ScaytAutoStartup)
						ScaytEngineLoad();
					if (FCK.EditMode == 0 && B && D)
						createScaytControl();
					ShowScaytState();
				}
			});
