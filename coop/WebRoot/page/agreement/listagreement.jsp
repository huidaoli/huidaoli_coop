	...
	<script type="text/javascript">
		$(function(){
			$('#tt').datagrid({
				...
				nowrap: false,
				striped: true,
				url:'agreement/getAjaxData.action',
				fitColumns:true,
				frozenColumns:[[
				...
				columns:[[
				    ...
				    {title:'乙方',field:'coopname',width:100},
					...
					{field:'opt',title:'操作',width:90,align:'center', 
						formatter:function(value,rec){
						    ...
						    if(rec.sxsj=='')
						    {
							    ...
							    html += "<span style='padding-right:10px;'><img src='../themes/icons/pencil.png'\ title='修改' style='cursor:pointer;border:1px solid #7eabcd;' onclick='getInfoById2(\"edit\","+rec.id+","+rec.type+")'></span>";
						         html += "<span style='padding-right:10px;'><img src='../themes/icons/valid.png'\ title='生效' style='cursor:pointer'onclick='passAgree("+rec.id+","+rec.type+")' ></span>";
						    }
						    ...
						    html += "<span style='padding-right:10px;'><img src='../themes/icons/tree_file.gif'\ title='合同文件管理' style='cursor:pointer'onclick=\"setcode('"+rec.code+"');getUpload('"+rec.code+"')\" ></span>";
							return html;     
						}
		...
		function getInfoById(opt)
		{
		     ...
		     if(rows.length!=1)
		     {
		         ...
		         loadEdit(strinfo,'agreement/getInfoById.action?opt='+opt+'&id='+ rows[0].id ,opt);
		...
		{
		     clearSelections();
		     window.open('getInfoById.action?opt='+opt+'&id='+id+"&type="+type, "_blank", '');
		}
		...
		function getInfoById2(opt,id,type)
		{
		     ...
		     {
		         //检查状态
		       $.get("agreement/checkState.action", {id: id},
					  function(data){
					    if(data==1)
					    { 
					        tAlert('信息','审核通过不能修改');
					    ...
					    {
					       loadEdit(strinfo,'agreement/getInfoById.action?opt='+opt+'&id='+id+"&type="+type,opt);
					    }
					  }
				);
		    ...
		    var rows = $('#tt').datagrid('getChecked');
		     if(rows.length==0){
		        ...
		        $.messager.confirm('信息','确认删除',function(r){
					if (r){
						$.post('agreement/delete.action',{ids:res},function(result){
							if (result.success){
...
<body>
    <table id="tt"></table> 
    <input type="hidden" id="contcodeid" name="contcodeid">
	<div id="tb" style="padding:5px;height:auto"> 
		<div style="margin-bottom:5px">
			...
			<br><br>
			<a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="loadEdit('新增合同','agreement/loadEdit.action','add')">新增</a>
