	...
	<script type="text/javascript">
		...
		$(function(){
			$('#tt').datagrid({
				...
				nowrap: false,
				striped: true,
				url:'scrollinfo/getAjaxData.action?cctype=${cctype}',
				fitColumns:true,
				frozenColumns:[[
				...
				columns:[[
				    {title:'图片名称',field:'name',width:100},
		...
		function loadEdit(tit,url,opt){
			var $div = $('<div id="dlg" class="easyui-dialog" style="width:500px;height:350px;padding:10px 20px" closed="true">');
			var options = {
	                title: tit,
					modal: true,
					shadow: false,
			...
			if(opt=='edit' || opt=='add')
			{
				settings = {
				    buttons:[{
							...
							iconCls:'icon-ok',
							handler:function(){
								saveOrUpdate('scrollinfo/saveOrUpdate.action?opt='+opt+'&cctype=${cctype}');
							}
						},{
							text:'关闭',
							handler:function(){
			...
			$('#fm').form('submit',{
				...
				success: function(result){
					...
					if (result.success){
						...
						tAlert('信息','保存成功');
						$('#dlg').dialog('close');
						$('#tt').datagrid('reload');	// reload the scrollinfo data
		...
		function getInfoById(opt)
		{
		     ...
		     if(rows.length!=1)
		     {
		         ...
		         var strinfo = opt=='edit'?'修改图片展示':'查看图片展示';
		         loadEdit(strinfo,'scrollinfo/getInfoById.action?opt='+opt+'&id='+ rows[0].id+'&cctype=${cctype}' ,opt);
		...
		function getInfoById2(opt,id)
		{
	         ...
	         var strinfo = opt=='edit'?'修改图片展示':'查看图片展示'
	         loadEdit(strinfo,'scrollinfo/getInfoById.action?opt='+opt+'&id='+id+'&cctype=${cctype}',opt);
		    ...
		    var rows = $('#tt').datagrid('getSelections');
		     if(rows.length==0){
		        ...
		        $.messager.confirm('信息','确认删除',function(r){
					if (r){
						$.post('scrollinfo/delete.action',{ids:res},function(result){
							if (result.success){
		...
		function queryinfo()
		{
		    ...
		    var typeval = $('#scrollinfo_type').val();
...
<body>
	...
	<div id="tb" style="padding:5px;height:auto">
		<div style="margin-bottom:5px">
			标题: <input name="scrollinfo.name" id="nameid">
			<a href="#" class="easyui-linkbutton" iconCls="icon-search" onclick="queryinfo()">查询</a><br>
			<a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="loadEdit('图片展示','scrollinfo/loadEdit.action?cctype=${cctype}','add')">新增</a>
