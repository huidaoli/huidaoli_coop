...
<head>
	...
	//contents/getOrgTreeData.action ../tree_data.json
		...
		function editOrgInfo(url,opt){
            ...
            $div.dialog({
				...
				buttons:[{
					...
					handler:function(){
					    var name = $('#nameid').val();
						saveOrg('contents/saveOrg.action?opt='+opt,opt,name,node);
					}
				},{
					text:'关闭',
					handler:function(){
		...
		function saveOrg(url,opt,text,node){
			$('#fm').form('submit',{
				url: url,
				onSubmit: function(){
		...
		function remove(){
			...
			else
			{
			    $.messager.confirm('信息','确认删除',function(r){
					if (r){
						$.post('contents/deleOrg.action',{id:node.id},function(result){
							if (result.success){
...
<body>
	<div id="mm" class="easyui-menu" style="width:120px;">
		<div onclick="editOrgInfo('contents/addOrg.action','add')" iconCls="icon-add">新增</div>
		<div onclick="editOrgInfo('contents/modOrg.action','mod')" iconCls="icon-edit">修改</div>
