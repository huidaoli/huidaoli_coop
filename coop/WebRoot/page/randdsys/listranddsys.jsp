	...
	<script type="text/javascript">
		...
		$(function(){
			$('#tt').datagrid({
				...
				nowrap: false,
				striped: true,
				url:'randdsys/getAjaxData.action',
				fitColumns:true,
				frozenColumns:[[
			...
			if(opt=='edit' || opt=='add')
			{
				settings = {
				    buttons:[{
							...
							iconCls:'icon-ok',
							handler:function(){
								saveOrUpdate('randdsys/saveOrUpdate.action?opt='+opt);
							}
						},{
							text:'关闭',
							handler:function(){
			...
			$('#fm').form('submit',{
				...
				success: function(result){
					...
					if (result.success){
						...
						tAlert('信息','保存成功');
						$('#dlg').dialog('close');
						$('#tt').datagrid('reload');	// reload the randdsys data
		...
		function getInfoById(opt)
		{
		     ...
		     if(rows.length!=1)
		     {
		         ...
		         loadEdit(strinfo,'randdsys/getInfoById.action?opt='+opt+'&id='+ rows[0].id ,opt);
		...
		function getInfoById2(opt,id)
		{
	         ...
	         var strinfo = opt=='edit'?'修改研发体系':'查看研发体系'
	         loadEdit(strinfo,'randdsys/getInfoById.action?opt='+opt+'&id='+id,opt);
		...
		function editImg(id)
		{
			...
			var options = {
					...
					modal: true,
					shadow: false,
					href:'randdsys/getImgsById.action?id='+id,
					closed: false,
					maximizable:true,
					onClose:function(){
					    //close方法触发的onClose事件,去调用destory方法
		...
		function reset()
	    {
             ...
             $.post('../trainingsys/resImg.action',{id:id},function(result){
							...
							if (result.result){
							    var ccid = $('#ccid').val();
								$('#dlg').dialog('refresh','randdsys/getImgsById.action?id='+ccid);
							} else {
								tAlert('信息','删除失败');
							}
						});
			...
			$('#eidtimg').form('submit',{
				...
				success: function(result){
					...
					if (result.result){
						...
						tAlert('信息','保存成功');
						//$('#dlg').refresh();
						$('#dlg').dialog('refresh','randdsys/getImgsById.action?id='+id);
		    ...
		    var rows = $('#tt').datagrid('getSelections');
		     if(rows.length==0){
		        ...
		        $.messager.confirm('信息','确认删除',function(r){
					if (r){
						$.post('randdsys/delete.action',{ids:res},function(result){
							if (result.success){
...
<input type="hidden" name="ccid" id="ccid">
	...
	<div id="tb" style="padding:5px;height:auto">
		<div style="margin-bottom:5px">
			...
			标题: <input name="randDSys.name" id="nameid">
			分类:   <s:select list="listDictBuss" headerKey="-1" headerValue="==请选择==" listKey="dictId" listValue="dictName" name="randDSys.type"></s:select>
			<a href="#" class="easyui-linkbutton" iconCls="icon-search" onclick="queryinfo()">查询</a>
			<br><br>
			<a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="loadEdit('研发体系','randdsys/loadEdit.action','add')">新增</a>
