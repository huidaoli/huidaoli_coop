...
<head>
	...
	<script>
		$(function(){
			$('#tt').datagrid({
				...
				columns:[[
					...
					{field:'opt',title:'定义表单',width:50,align:'center', 
						formatter:function(value,rec){
						    //alert(rec.id);
							return "<span style='padding-right:10px;'><img src='../themes/icons/pencil.png'\ title='定义表单' style='cursor:pointer;border:1px solid #7eabcd;' onclick='getFlowformById2(\"edit\","+rec.id+")'></span>";
						}
		...
		function loadFlowform(tit,url,opt){
			
			var $div = $('<div id="dlg" class="easyui-dialog" style="width:650px;height:480px;padding:10px 10px" closed="true">');
			...
			if(opt=='edit' || opt=='add')
			{
				settings = {
				    buttons:[{
							...
							iconCls:'icon-ok',
							handler:function(){
								saveFlowform('jbpm/save.action?opt='+opt);
							}
						},{
							text:'关闭',
							handler:function(){
			...
			$('#fm').form('submit',{
				...
				success: function(result){
					...
					if (result.success){
						...
						tAlert('信息','保存成功');
						$('#dlg').dialog('close');
						$('#tt').datagrid('reload');	// reload the Flowform data
		...
		function getFlowformById(opt)
		{
		     ...
		     if(rows.length!=1)
		     {
		         ...
		         var strinfo = opt=='edit'?'表单定义':'表单定义';
		         loadFlowform(strinfo,'jbpm/loadFlowformById.action?opt='+opt+'&id='+ rows[0].id ,opt);
		        
		     }
		}
		
		
		function getFlowformById2(opt,id)
		{
		     clearSelections();
		     //window.event.cancelBubble = true;
	         var strinfo = opt=='edit'?'表单定义':'表单定义'
	         loadFlowform(strinfo,'jbpm/addFormInput.action?opt='+opt+'&id='+id,opt);
		     
		}
		
		function deleteFlowform(){
		    ...
		    var rows = $('#tt').datagrid('getSelections');
		     if(rows.length==0){
		        ...
		        $.messager.confirm('信息','确认删除',function(r){
					if (r){
						$.post('jbpm/deleteFlowform.action',{ids:res},function(result){
							if (result.success){
								$('#tt').datagrid('reload');	// reload the Flowform data
							} else {
								tAlert('信息','删除失败');
