	...
	<script type="text/javascript">
		...
		$(function(){
			$('#tt').datagrid({
				...
				nowrap: false,
				striped: true,
				url:'practiceresult/getAjaxData.action',
				fitColumns:true,
				frozenColumns:[[
			...
			if(opt=='edit' || opt=='add')
			{
				settings = {
				    buttons:[{
							...
							iconCls:'icon-ok',
							handler:function(){
								saveOrUpdate('practiceresult/saveOrUpdate.action?opt='+opt);
							}
						},{
							text:'关闭',
							handler:function(){
			...
			$('#fm').form('submit',{
				...
				success: function(result){
					...
					if (result.success){
						...
						tAlert('信息','保存成功');
						$('#dlg').dialog('close');
						$('#tt').datagrid('reload');	// reload the practiceresult data
		...
		function getInfoById(opt)
		{
		     ...
		     if(rows.length!=1)
		     {
		         ...
		         var strinfo = opt=='edit'?'修改项目成果':'查看项目成果';
		         loadEdit(strinfo,'practiceresult/getInfoById.action?opt='+opt+'&id='+ rows[0].id ,opt);
		...
		function getInfoById2(opt,id)
		{
	         ...
	         var strinfo = opt=='edit'?'修改项目成果':'查看项目成果'
	         loadEdit(strinfo,'practiceresult/getInfoById.action?opt='+opt+'&id='+id,opt);
		    ...
		    var rows = $('#tt').datagrid('getSelections');
		     if(rows.length==0){
		        ...
		        $.messager.confirm('信息','确认删除',function(r){
					if (r){
						$.post('practiceresult/delete.action',{ids:res},function(result){
							if (result.success){
		...
		function queryinfo()
		{
		    ...
		    var typeval = $('#practiceResult_type').val();
...
<body>
	...
	<div id="tb" style="padding:5px;height:auto">
		<div style="margin-bottom:5px">
			...
			标题: <input name="practiceResult.name" id="nameid">
			分类:   <s:select list="listDictBuss" headerKey="-1" headerValue="==请选择==" listKey="dictId" listValue="dictName" name="practiceResult.type"></s:select>
			<a href="#" class="easyui-linkbutton" iconCls="icon-search" onclick="queryinfo()">查询</a>
			<br><br>
			<a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="loadEdit('项目成果','practiceresult/loadEdit.action','add')">新增</a>
