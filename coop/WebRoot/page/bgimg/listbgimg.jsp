	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$('#tt').datagrid({
				...
				nowrap: false,
				striped: true,
				url:'bgimg/getAjaxData.action?cctype=${cctype}',
				fitColumns:true,
				frozenColumns:[[
					...
					{field:'opt',title:'操作',width:80,align:'center', 
						formatter:function(value,rec){
							...
							return "<span style='padding-right:10px;'><img src='../themes/icons/pencil.png'\ title='修改' style='cursor:pointer;border:1px solid #7eabcd;' onclick='getInfoById2(\"edit\","+rec.id+")'></span>"+
							   ...
							   "<span style='padding-right:10px;'><img src='../themes/icons/"+bg+"'\ title='设为背景' style='cursor:pointer' onclick='usedBg("+rec.id+")' ></span>";
						}
		...
		{
		     clearSelections();
		     $.post('bgimg/usedBgImg.action',{id:id,type:${cctype}},function(result){
							if (result.success){
			...
			if(opt=='edit' || opt=='add')
			{
				settings = {
				    buttons:[{
							...
							iconCls:'icon-ok',
							handler:function(){
								saveOrUpdate('bgimg/saveOrUpdate.action?opt='+opt+'&cctype=${cctype}');
							}
						},{
							text:'关闭',
							handler:function(){
			...
			$('#fm').form('submit',{
				...
				success: function(result){
					...
					if (result.success){
						...
						tAlert('信息','保存成功');
						$('#dlg').dialog('close');
						$('#tt').datagrid('reload');	// reload the bgimg data
		...
		function getInfoById(opt)
		{
		     ...
		     if(rows.length!=1)
		     {
		         ...
		         var strinfo = opt=='edit'?'修改背景图片':'查看背景图片';
		         loadEdit(strinfo,'bgimg/getInfoById.action?opt='+opt+'&id='+ rows[0].id+'&cctype=${cctype}' ,opt);
		...
		function getInfoById2(opt,id)
		{
	         ...
	         var strinfo = opt=='edit'?'修改背景图片':'查看背景图片'
	         loadEdit(strinfo,'bgimg/getInfoById.action?opt='+opt+'&id='+id+'&cctype=${cctype}',opt);
		    ...
		    var rows = $('#tt').datagrid('getSelections');
		     if(rows.length==0){
		        ...
		        $.messager.confirm('信息','确认删除',function(r){
					if (r){
						$.post('bgimg/delete.action',{ids:res},function(result){
							if (result.success){
								$('#tt').datagrid('reload');	// reload the  data
							} else {
								tAlert('信息','删除失败');
...
<body>
	...
	<div id="tb" style="padding:5px;height:auto">
		<div style="margin-bottom:5px">
			...
			名称: <input name="bgImg.name" id="nameid">
			<a href="#" class="easyui-linkbutton" iconCls="icon-search" onclick="queryinfo()">查询</a>
			<br><br>
			<a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="loadEdit('背景图片','bgimg/loadEdit.action?cctype=${cctype}','add')">新增</a>
