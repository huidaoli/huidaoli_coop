package com.cantodo.content.practicesys.service.impl;
...
import com.cantodo.content.persistence.PracticeSysMapper;
import com.cantodo.content.practicesys.service.PracticeSysService;

@Service("practiceSysServiceImpl")
public class PracticeSysServiceImpl implements PracticeSysService
{

    private Log logger = LogFactory.getLog(PracticeSysServiceImpl.class);

    @Autowired
    private DictBussManager dictBussManager;

    @Autowired
    private PracticeSysMapper practiceSysMapper;
    ...
    public String getList(Map param)
    {
				...
				new JsonValueProcessor() {
					...
					public Object processObjectValue(String key, Object value,
						...
						if (key.equals("type")) {
							...
							String res = "";
							List<DictBuss> listDictBuss = listDictBuss(25);
		...
		List<PracticeSys> list = null;
		int total = 0;
		try {
			list = practiceSysMapper.getAllInfo(param);
			total = practiceSysMapper.getCounts(param);
    ...
    public void add(PracticeSys practiceSys)
    {
    	logger.debug("enter add");

        practiceSysMapper.insert(practiceSys);
    ...
    public void update(PracticeSys practiceSys)
    {
    	logger.debug("enter update");

    	practiceSysMapper.update(practiceSys);
  		
        logger.debug("exit update");
    }

    public PracticeSys getInfoById(int id)
    {
    	logger.debug("enter getInfoById");
    	
    	PracticeSys r =  practiceSysMapper.getInfoById(id);
    ...
    public void delete(String strids)
    {
        ...
        for (int id : ids)
        {
        	practiceSysMapper.delete(id);
        }
  		
        logger.debug("exit delete");
    }
    
    public List<PracticeSys> getAllInfo(Map pata)
    {
    	logger.debug("enter getAllInfo");
    	
    	 List<PracticeSys> r = practiceSysMapper.getAllInfo(pata);
    ...
    public List<Map> getImgsById(int id)
    {
    	logger.debug("enter getImgsById");
    	 ...
    	 List<Map> lm=  practiceSysMapper.getImgsById(id);
    ...
    public int getCounts2()
    {
    	...
    	int t =  practiceSysMapper.getCounts2();
